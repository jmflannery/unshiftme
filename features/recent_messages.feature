...
Feature: Messages
  ...
  Scenario: Recent messages: When the current user loads the messaging page, messages
    sent to the current user's desks, while the desk was vacant during the last 24 hours
    will be visible. Messages sent to the user's desk while another user was working
    the desk during the last 24 hours will not be visible
    Given the following user records
      | user_name | id |
      | sam       | 3  |
      | jeff      | 1  |
    ...
    And I should see unread recieved message 2 "Anyone??" from desk "CUSS" user "bob" one time
