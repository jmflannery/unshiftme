...
describe Workstation do
  ...
  before(:each) do
    ...
    @aml = Workstation.create(name: "AML / NOL", abrev: "AML", job_type: "td")
    @ydctl = Workstation.create(name: "Yard Control", abrev: "YDCTL", job_type: "ops")
    @ydmstr = Workstation.create(name: "Yard Master", abrev: "YDMSTR", job_type: "ops")
    @glhs = Workstation.create(name: "Glasshouse", abrev: "GLHSE", job_type: "ops")
  end
  ...
  describe "#view_class" do
    ...
    context "when the workstation is a recipient of the given user" do
      before { @recip = FactoryGirl.create(:recipient, user: user, workstation_id: cusn.id) }
      it "should have the right view class" do
